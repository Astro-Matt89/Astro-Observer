...
class ImagingScreen(BaseScreen):
    ...
    def __init__(self, state_manager):
        ...
        # Time controller (fluido, condiviso con skychart)
    ...
    def _sky_signal_allsky(self, exp_s, universe):
        """Render full-sky hemisphere per allsky cameras."""
        jd = self._tc.jd
    ...
    def _expose(self):
        ...
        # Aggiorna stato atmosferico dal tempo simulato
        now_utc = self._tc.utc
    ...
    def update(self, dt):
        """Avanza il tempo simulato e aggiorna live preview periodicamente."""
        self._tc.step(dt)
    ...
    def handle_input(self, events) -> Optional[str]:
        ...
        for event in events:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # Controlli tempo (F-keys per non conflittare con stretch ,/.)
                ...
                elif k == pygame.K_F3:            self._tc.set_speed_idx(5)   # 1h/s
                elif k == pygame.K_F4:            self._tc.set_speed_idx(6)   # 1d/s
                elif k == pygame.K_F5:            self._tc.set_speed_idx(7)   # 1wk/s
                elif k == pygame.K_F6:            self._tc.reverse()
    ...
    def _draw_header(self, surface, W):
        pygame.draw.rect(surface, (0, 18, 10), (0, 0, W, 80))
        pygame.draw.line(surface, (0, 100, 50), (0, 80), (W, 80), 1)
        ...
        # Ora simulata + indicatore velocità
        sim_col = (0, 160, 200) if not self._tc.paused else (180, 180, 60)
        sim_str = (f"{self._tc.utc.strftime('%Y-%m-%d  %H:%M:%S UTC')}  "
                   f"[ {self._tc.speed_label} ]   "
                   f"[Spc]pause  [F1]RT  [F2]60×  [F3]1h/s  [F4]1d/s  [F5]1wk/s  [F6]rev")
        surface.blit(fs.render(sim_str, True, sim_col), (12, 50))

        info = self.renderer.get_info()
        cam_info = (f"Camera: {self.camera.spec.name}  |  "
                    + (f"FOV 180° AllSky" if self.is_allsky else
        ...
        surface.blit(fs.render(cam_info, True, (0, 130, 65)), (12, 64))
    ...
    def _draw_viewer(self, surface, LP, W, H):
        ...
        img_rect = pygame.Rect(px+4, 102, pw-8, ph - 130 - hist_h)
